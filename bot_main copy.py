...
class BOT():
    ...
    def get_channel_list(self):
        api_url = self.URL+f"/api/v3/channel/list?guild_id={self.SERVER_ID}"
        response = requests.get(api_url,headers=self.HEADERS)
        if response.status_code ==200:
            print(response.json())
    def get_server_list(self):
        api_url = self.URL+"/api/v3/guild/list"
        response = requests.get(api_url,headers=self.HEADERS)
        if response.status_code ==200:
            print(response.json())
    ...
    def get_msg_list(self):
        ...
        api_url=self.URL+f"/api/v3/message/list?target_id={self.CHANNEL_ID}"
        try :
            response = requests.get(api_url,headers=self.HEADERS)
            if response.status_code == 200:
    ...
    def post_msg(self,content,quete = ''):
        
        api_url = self.URL+'/api/v3/message/create'
        data = {
            "target_id":f"{self.CHANNEL_ID}",
            "type": 9,
            "content":content,
            "quote":quete
        }
        ...
        try :
            response = requests.get(api_url,data=data,headers=self.HEADERS)
            if response.status_code == 200:
                response_json = response.json()
                ...
                if msg_item['create_at'] > last_msg_time:
                        ...
                        if id_list == '':
                            ...
                            if responce_rows == -1 :
                                ...
                                for priceInfo in responce_rows:
                                    ...
                                    update_time = BOT.UTCstringToBeijing(priceInfo['buy_price_max_date'])
                    ...
                    #找到信息后回复该玩家
                        ...
                        #     responce_content = '查询结果为空，原因可能有:\n数据库暂未更新\n机器人本地化信息缺失\n查询内容有误'
                            ...
                            self.post_msg(responce_content,msg_item['id'])
